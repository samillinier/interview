...
/** Add an installer to the curated Report list. */
export async function POST(request: NextRequest) {
  try {
    ...
    if (!installerId) {
      return NextResponse.json({ error: 'installerId is required' }, { status: 400, headers: noStoreHeaders })
    }
    ...
    const dup = await prisma.installerTracking.findFirst({
      where: { type: 'report_manual', installerId },
    })
    if (dup) {
      return NextResponse.json(
        { error: 'This installer is already on the report.' },
        { status: 409, headers: noStoreHeaders }
      )
    }

    const maxRow = await prisma.installerTracking.aggregate({
      where: { type: 'report_manual' },
    ...
    const row = await prisma.installerTracking.create({
      data: {
        ...
        type: 'report_manual',
        title: 'Report row',
        description: 'Installer included on the admin Report page.',
        status: 'pending',
        priority: 'normal',
        category: 'report',
        metadata: { reportManual: true },
    ...
    console.error('report manual POST:', error)
    return NextResponse.json(
      { error: 'Failed to add installer to report', details: error.message },
