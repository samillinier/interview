...
export async function DELETE(
  ...
  try {
    ...
    if (existing.type !== 'matrix_manual') {
      return NextResponse.json({ error: 'Not a matrix row' }, { status: 400, headers: noStoreHeaders })
    }

    await prisma.installerTracking.delete({ where: { id } })
    return NextResponse.json({ success: true }, { headers: noStoreHeaders })
  } catch (error: any) {
    console.error('onboarding-matrix manual DELETE:', error)
    return NextResponse.json(
      { error: 'Failed to remove manual row', details: error.message },
