...
export async function GET(
  ...
  try {
    ...
    console.error('Error fetching W-9 agreement:', error)
...
export async function PUT(
  ...
  try {
    ...
    const agreement = await prisma.installerAgreement.upsert({
      where: { installerId_type: { installerId, type: AGREEMENT_TYPE } },
      update: { status, payload, signedAt },
      create: { installerId, type: AGREEMENT_TYPE, status, payload, signedAt },
      select: SAFE_AGREEMENT_SELECT,
    })
    ...
    console.error('Error saving W-9 agreement:', error)
