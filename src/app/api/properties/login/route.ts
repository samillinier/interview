...
export async function POST(request: NextRequest) {
  try {
    ...
    // Find property by username or email (for backward compatibility)
    ...
    // Block login if property is deactivated
    if ((property.status || '').toLowerCase() === 'deactive') {
      return NextResponse.json(
    ...
    // Verify password
    const isValidPassword = await bcrypt.compare(password, property.passwordHash)
