...
export default function JobsReportsPage() {
  ...
  const getDisplayDate = (r: CilioJobRecord): string | null => {
    return r.scheduledInstallDate
      || r.cilioFields?.dateInformation?.desiredInstallDate
      || r.cilioFields?.schedulingInformation?.scheduleDate
      || r.cilioFields?.currentOrderStatusDate
      || null
  }
              ...
              <div className="bg-white rounded-2xl border border-slate-200/80 shadow-md overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="w-full">
                    ...
                    <tbody>
                      {records.map(record => {
                        const poAmount = record.cilioFields?.poAmount ?? null
