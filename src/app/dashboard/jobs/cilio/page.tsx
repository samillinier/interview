  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 0; i < allJobs.length; i += batchSize) {
        const batch = allJobs.slice(i, i + batchSize).map(j => ({
          orderNumber: j.orderNumber,
          orderStatusDescription: j.orderStatusDescription ?? null,
          jobType: (j.orderStatusDescription?.toLowerCase().includes('chargeback') ? 'chargeback' : 'scheduled') as 'scheduled' | 'chargeback',
          storeNumber: j.storeNumber ?? null,
          storeName: j.storeName ?? null,
          laborCategoryDescription: j.laborCategoryDescription ?? null,
          workroom: (j as any).workroomByStore ?? null,
          scheduledInstallDate: (j as any).scheduledInstallDate ?? null,
          measureDate: (j as any).measureDate ?? null,
          bookingDate: (j as any).bookingDate ?? null,
          installerId: (j as any)._installer?.id ?? null,
          installerName: (j as any)._installer?.name ?? null,
          cilioPayload: j,
        }))
        const res = await fetch('/api/cilio/jobs/sync', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      setSaveResult({ synced: totalSynced })
