...
function getInstallerSearchHeadsUp(installer: Installer, role: string) {
  ...
  // Compliance status
  if (installer.complianceStatus === 'COMPLIANT') {
    items.push({ type: 'compliant', label: 'Compliant', detail: 'All documents are valid and up to date' })
  } else if (installer.complianceStatus === 'NOT_COMPLIANT') {
    items.push({ type: 'not_compliant', label: 'Not Compliant', detail: 'Some documents may be expired or missing' })
  } else if (installer.complianceStatus === 'IN_PROGRESS') {
    items.push({ type: 'in_progress', label: 'In Progress', detail: 'Document review is still underway' })
  }
                      ...
                      <div className="flex flex-col gap-2">
                        {headsUpItems.map((item, idx) => (
                          ...
                          >
                            ...
                            ) : item.type === 'compliant' || item.type === 'not_compliant' || item.type === 'in_progress' ? (
