...
export function getCilioHeaders(): HeadersInit {
...
/** Returns only the auth header (no Content-Type), for routes that handle binary responses. */
export function getCilioAuthHeader(): HeadersInit {
  if (!CILIO_SUBSCRIPTION_KEY) {
    throw new Error("CILIO_SUBSCRIPTION_KEY is not configured. Please set it in your environment variables.")
  }
  return {
